...
class OTB(object):
    ...
    __subset2 = ['Basketball', 'BlurOwl', 'Girl2',
                'Human2', 'Human7', 'Jogging', 'Lemming',
                'Liquor', 'Sylvester', 'Walking2']

    __otb15_seqs = __tb100_seqs

    __version_dict = {
        'hyperparam': __subset2,
        2013: __otb13_seqs,
        2015: __otb15_seqs,
    ...
    def __init__(self, root_dir, version=2015, download=True):
        ...
        self.anno_files = sorted(list(chain.from_iterable(glob.glob(
            os.path.join(root_dir, s, 'groundtruth_rect.txt')) for s in valid_seqs)))
